(ns nido.coordinator.record.runs
...
(def Run
  [:map {:closed true}
   ...
   ;; nido.coordinator.lane.pipeline's question and is derived, while this is a fact about one
...
(defn teardown-session-for-run!
  "Reclaim the session a Run spawned, once the Run reaches a resolved-terminal
   ...
   coordinator-owned ephemeral surface. The run goes :done (to free the trigger's
   slot) once the burst finishes — the Notion leg when /continue-ticket sets the
   ticket :implementing, the GitHub leg immediately — but the session must OUTLIVE
