...
(def ^:private shipping-doctrine-instructions
  (str "## Shipping doctrine\n"
       ...
       "\n"
       "That goal decides what leaves the branch and what waits for a deploy.\n"
       "It does NOT decide how a change is layered. Sized to what a reviewer\n"
       "can hold, every split looks like a gain — a smaller piece is always\n"
       "easier to hold — and the stack fills with layers too small to mean\n"
       "anything. A change is layered the way its code is built: by level of\n"
       "abstraction (see *Vertical*).\n"
       "\n"
       ...
       "  what was rejected and why, the strata it touches, and `:baseline`\n"
       ...
       "\n"
       "### The words, and which boundaries survive a landing\n"
       "\n"
       "A layer is a DEVELOPMENT-TIME device. `/land` collapses the reviewed\n"
       "stack into one commit before it merges, so no layer boundary is ever a\n"
       "merge boundary and no layer survives in the history. The levels it\n"
       "read survive, because they are in the code; the cut does not. One rule\n"
       "falls out, and the table below is that rule applied:\n"
       "\n"
       "> **A word that names something which dies at land time may never be the\n"
       "> word for something that survives it.**\n"
       "\n"
       "| word | what it names | survives landing |\n"
       "|---|---|---|\n"
       "| **boundary** | a module or interface boundary in the software — a hidden decision, in Parnas's sense | yes |\n"
       "| **seam** | a change's own deliberate incompleteness, declared in `:seams` with what closes it | yes, until its closure lands |\n"
       "| **stratum** | a level of the code: the modules providing a vocabulary the levels above are written in — declared in `canvas/`, named by a design's `:strata` | yes |\n"
       "| **cut** | how one change is divided into layers — what `/stack` draws from the design's strata | **no** — collapsed at `/land` |\n"
       "| **phase** | a landing boundary: the running system has to live in this state | yes — it is a deploy |\n"
       "| **injection seam** | a place a test may substitute behaviour without editing in place — Feathers's sense | yes |\n"
       "\n"
       "The last row is named because it is the commonest use of the word in\n"
       "this codebase and it is NOT the first: `:seams` on a design record and a\n"
       "redef point in a namespace are different things that happen to share a\n"
       "noun. Both survive a landing, so neither breaks the rule — say which you\n"
       "mean when the sentence does not make it obvious, and never let a third\n"
       "sense in.\n"
       "\n"
       "**Re-cutting buys nothing durable.** The code lands whole whichever way\n"
       "it was cut, so a cut that misreads the levels costs only the reading it\n"
       "was drawn for. A layering observation has to name something that is\n"
       "not packaging: a claim that is untrue, work no reviewer held, a layer\n"
       "that would not stand up at its own tip. \"It would read better cut\n"
       "elsewhere\" is not a finding, and is never worth a round.\n"
       "\n"
       "**So a concern about the cut, alone, never blocks.** Not a design round,\n"
       "not a code review. Say it, carry it, and let the work proceed — the\n"
       "layers are gone before anything lands, and a cut nobody re-litigated is\n"
       "not a defect the merged history can inherit.\n"
       "\n"
       "### Vertical — layers within one story\n"
       "\n"
       "The design names the **strata** a change touches — the levels of the\n"
       "code, in stratified design's sense (Abelson & Sussman): each built\n"
       "from what the level below provides, each providing what the level\n"
       "above is written in. Strata survive the landing; the cut does not, so\n"
       "the design states no cut. `/stack` draws it at stack time: **by default\n"
       "one layer per stratum the change writes in, floor first**, plus the\n"
       "three boundaries below that are not levels. Each layer is a\n"
       "**level of abstraction** read off those strata, never a new claim\n"
       "about the code.\n"
       "\n"
       "- **The level test.** Name what this layer provides — a function, a\n"
       "  type, a schema, a protocol — that the layer above uses by its\n"
       "  interface, without reading its body. Nothing to name, no boundary. A\n"
       "  layer that only forwards what the layer below already accepts, or\n"
       "  renames on the way through, provides nothing: fold it into the layer\n"
       "  it serves.\n"
       "- **The code has the levels; the cut reads them.** A change written at\n"
       "  one level is one layer however large it is. A level is not a pipeline\n"
       "  stage — parse, transform, persist over one representation is one\n"
       "  level. A boundary the code does not have is a design change: build it\n"
       "  in the code, never only in the packaging. Cut against the CODE, never\n"
       "  against the call chain you expect to find.\n"
       "- **The test does not ratchet.** \"Could this be separated?\" is more\n"
       "  true after every split. Split a level in two and one half provides\n"
       "  nothing the other is written in, which fails the test.\n"
       "- **Three boundaries are worth drawing although they are not levels,\n"
       "  and only three.** *Introduce, then remove* — removing what a new\n"
       "  abstraction replaces is its own layer, always on top.\n"
       "  *Refactor before change* — a behaviour-preserving restructure sits\n"
       "  below the change it makes easy. *Mechanical sweep apart* — one\n"
       "  uniform transformation with no special cases stays out of any layer\n"
       "  carrying judgment. Size, review mode and which specialist reads it\n"
       "  split nothing.\n"
       "- **Order by dependency.** It falls out of the test: if code in layer A\n"
       "  depends on layer B, B is in the same layer or lower.\n"
       ...
       "  `mechanical`, `structural`, `behavioral`, the strongest thing the\n"
       "  layer does — plus a review brief (Claims / Verify / Lane / Out of\n"
       "  scope). A level's claim is what it provides.\n"
       "- **One-sentence PR titles, no \"and\".** A title that needs \"and\"\n"
       "  names a layer's parts; name what it provides. If there is no one\n"
       "  thing, it is two layers.\n"
       "- **Don't stack a change with one level.** Ship one plain PR — and a\n"
       "  change under ~200 lines is one PR even when it has two.\n"
       "- **When a layer resists on contact, suspect the cut.** Collapsing one\n"
       "  boundary while executing is a local fix. Collapsing a second is a\n"
       "  finding about the decomposition — amend the design record then, not at\n"
       "  the end.\n"
