...
(def ^:private shipping-doctrine-instructions
  (str "## Shipping doctrine\n"
       ...
       "\n"
       "Before either cut, say what the change is **for**. A finding is only\n"
       "meaningful relative to an intent: with none stated, \"this is wrong\" and\n"
       "\"this is not what we decided\" are indistinguishable, and every reviewer\n"
       "re-derives a different design from the diff.\n"
       "\n"
       "- **One design record per workstream**, in the ledger — shape, invariants,\n"
       "  what was rejected and why, the intended layer cut. Claims about\n"
       "  structure, never a step list. See `/design`.\n"
       ...
       "  record. Never quietly patch around it.\n"
       "\n"
       "Work is then decomposed along two axes, and every unit lands in one of\n"
       "four destinations:\n"
       "\n"
       ...
       "- **Past ~3 spin-outs from one branch the ticket was mis-scoped.** Stop\n"
       "  and revisit the decomposition.\n"
       "\n"
