...
(def ^:private shipping-doctrine-instructions
  (str "## Shipping doctrine\n"
       ...
       "flight.\n"
       "\n"
       "That attention is spent on two things, not one: what each batch claims,\n"
       "and whether the decomposition into batches is the right one. Only the\n"
       "first shrinks when you split.\n"
       "\n"
       ...
       "merge boundary and no layer survives in the history. Its whole currency\n"
       "is reviewer attention. One rule falls out, and the table below is that\n"
       "rule applied:\n"
       "\n"
       ...
       "sense in.\n"
       "\n"
       "**Re-cutting buys nothing durable.** A better cut saves the attention of\n"
       "the reviewers currently reading it — and by the time anyone can report\n"
       "that the cut is wrong, that attention is already spent. So a layering\n"
       "observation has to name what it costs THIS review: a claim that cannot\n"
       "be judged, work no reviewer held, a layer that would not stand up alone.\n"
       "\"The story would read better\" is not a finding, and is never worth a\n"
       "round.\n"
       "\n"
       ...
       "- **Layers are read bottom to top, as a story.** They should narrate how\n"
       "  the change was made: each one a coherent, self-contained move a reader\n"
       "  could stop at and still know where they are. That is the test — not\n"
       "  whether a layer COULD be separated, but whether separating it tells the\n"
       "  reader something.\n"
       "- **The cut is reviewed too, and every layer adds to that review.** A\n"
       "  reviewer holds two questions at once: does this layer's claim stand,\n"
       "  and is this the right decomposition? The second grows with the count —\n"
       "  n layers is n claims to judge plus n boundaries to justify — so a split\n"
       "  is never free, and layers that are each individually defensible can\n"
       "  still be the wrong cut. Nothing in a decomposition check ever rewards\n"
       "  merging; that asymmetry is yours to correct, so make the merge case\n"
       "  against yourself before you make the split one.\n"
       ...
       "  the call chain you expect to find; a plan drawn from an assumed shape\n"
