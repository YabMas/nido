...
(defn ^{:malli/schema [:=> [:cat :map] :string]}
  ...
  (str
   ...
   "                      This check runs BOTH ways. `can it be separated?` is\n"
   "                      always more true after a split, so asking only that\n"
   "                      ratchets a design toward more layers every round and\n"
   "                      never back. Ask also what each boundary BUYS a reader:\n"
   "                      a layer that only forwards what the layer below\n"
   "                      already accepts, or that a reviewer would not stop at,\n"
   "                      is a defect in the cut and not a small layer. Read the\n"
   "                      layers as a story, bottom to top: each should be a\n"
   "                      self-contained move a reader could stop at and still\n"
